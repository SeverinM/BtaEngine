...
namespace Bta
{
	namespace Graphic
	{
		...
		VkCommandBuffer* CommandFactory::CreateCommand()
		{
			...
			if (vkAllocateCommandBuffers(*Bta::Graphic::Globals::g_pDevice->GetLogicalDevice(), &oAllocateInfo, pCmdBuffer) != VK_SUCCESS)
			{
		...
		VkCommandBuffer CommandFactory::CreateDrawCommand(DrawDesc& oDesc)
		{
			...
			if (vkAllocateCommandBuffers(*Bta::Graphic::Globals::g_pDevice->GetLogicalDevice(), &oAllocateInfo, &oCmdBuffer) != VK_SUCCESS)
			{
				throw std::runtime_error("Error allocating command buffer");
			}
