	...
	namespace Graphic
	{
		...
		std::shared_ptr<BasicBuffer> BufferedTransform::MergeTransform(std::vector<std::shared_ptr<Transform>> oTrsf, VkBufferUsageFlags eUsage, std::vector<std::shared_ptr<BufferedTransform>>& oBufferedTransform)
		{
			...
			for (std::shared_ptr<Transform> xTrans : oTrsf)
			{
				...
				oBufferedTransform.push_back(std::make_shared<BufferedTransform>(BufferedTransform(mInput, iOffset, std::static_pointer_cast<Buffer>(xBuffer), Bta::Graphic::Globals::g_pDevice)));
		...
		{
			void* pData = &m_mModel;
			m_xBuffer->CopyFromMemory(pData, Bta::Graphic::Globals::g_pDevice, m_iOffset, sizeof(glm::mat4));
		}
