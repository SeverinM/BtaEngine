...
namespace Bta
{
	namespace Graphic
	{
		...
		void DescriptorPool::Create(Desc& oDesc)
		{
			...
			if (vkCreateDescriptorPool(*Bta::Graphic::Globals::g_pDevice->GetLogicalDevice(), &oPoolInfo, nullptr, &m_oPool) != VK_SUCCESS)
			{
		...
		{
			vkDestroyDescriptorPool(*Bta::Graphic::Globals::g_pDevice->GetLogicalDevice(), m_oPool, nullptr);
		}
		...
		void DescriptorPool::WriteDescriptor(DescriptorSetWrapper* pDescriptorSet)
		{
			...
			vkUpdateDescriptorSets(*Bta::Graphic::Globals::g_pDevice->GetLogicalDevice(), static_cast<uint32_t>(oWriteDescriptors.size()), oWriteDescriptors.data(), 0, nullptr);
		}
		...
		void DescriptorPool::CreateDescriptorSet(std::vector<VkDescriptorSet>& oOutput, int iSize, const VkDescriptorSetLayout& oLayout)
		{
			...
			if (vkAllocateDescriptorSets(*Bta::Graphic::Globals::g_pDevice->GetLogicalDevice(), &oAllocInfo, oOutput.data()) != VK_SUCCESS)
			{
		...
		void DescriptorPool::CreateDescriptorSet(VkDescriptorSet& oOutput, const VkDescriptorSetLayout& oLayout)
		{
			...
			if (vkAllocateDescriptorSets(*Bta::Graphic::Globals::g_pDevice->GetLogicalDevice(), &oAllocInfo, &oOutput) != VK_SUCCESS)
			{
				throw std::runtime_error("Failed to allocate descriptor sets");
			}
		}
