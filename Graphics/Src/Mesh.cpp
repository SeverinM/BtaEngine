...
namespace Bta
{
	namespace Graphic
	{
		...
		void Mesh::ConvertToVerticesBuffer(BufferElementsFlag eFlags, bool bIncludeIndexes)
		{
			...
			m_xCachedVerticesBuffer = std::shared_ptr<Buffer>(GPUMemory::GetInstance()->AllocateMemory(oDesc));
			m_xCachedVerticesBuffer->CopyFromMemory(oBytes.data(), Bta::Graphic::Globals::g_pDevice);
			...
			if (bIncludeIndexes)
			{
				...
				m_xCachedIndexesBuffer = std::shared_ptr<Buffer>(GPUMemory::GetInstance()->AllocateMemory(oDescIndex));
				m_xCachedIndexesBuffer->CopyFromMemory(m_oIndexes.data(), Bta::Graphic::Globals::g_pDevice);
			}
		}
